...
RegisterCommand("debug",function(source,args,rawCommand)
	local user_id = vRP.getUserId(source)
	if user_id then
		if vRP.hasRank(user_id,"Admin",40) or vRP.hasPermission(user_id,"Debug") then
			if args[1] then
				local nSource = vRP.getUserSource(parseInt(args[1]))
				local nuser_id = vRP.getUserId(nSource)
				if nSource and nuser_id then
					if vRPC.getHealth(nSource) > 101 and not vCLIENT.getHandcuff(nSource) then
						local health = vRPC.getHealth(nSource)
						local armour = vRPC.getArmour(nSource)
						local weapons = vRPC.getWeapons(nSource)
						local model = vRPC.ModelPlayer(nSource)
						if model then
							vRPC.applySkin(nSource,model)
							vRP.updateSelectSkin(nuser_id,model)
						end

						if health then
							vRPC.setHealth(nSource,health)
							vRPC.setArmour(nSource,armour)
						end

						if weapons then
							vRPC.giveWeapons(nSource,weapons,true)
						end

						local value = vRP.getUData(nuser_id,"Character")
						local custom = json.decode(value) or {}
						if custom then
							TriggerClientEvent("vrp_character:updateCharacter",nSource,custom,false)
						end

						-- VRP_SKINSHOP
						local playerData = vRP.getUData(nuser_id,"Clothings")
						local resultData = json.decode(playerData)
						if resultData == nil then
							resultData = "clean"
						end
						TriggerClientEvent("vrp_skinshop:skinData",nSource,resultData)

						Citizen.Wait(1000)

						-- VRP_TATTOOS
						local tattooData = vRP.getUData(nuser_id,"Tattoos")
						local result = json.decode(tattooData)
						if result then 
							TriggerClientEvent("vrp_tattoos:setTattoos",nSource,result)
						end
						TriggerEvent("vrp_inventory:Cancel",nSource)
					end
				end
			else
				if vRPC.getHealth(source) > 101 and not vCLIENT.getHandcuff(source) then
					local health = vRPC.getHealth(source)
					local armour = vRPC.getArmour(source)
					local weapons = vRPC.getWeapons(source)
					local model = vRPC.ModelPlayer(source)
					if model then
						vRPC.applySkin(source,model)
						vRP.updateSelectSkin(user_id,model)
					end

					if health then
						vRPC.setHealth(source,health)
						vRPC.setArmour(source,armour)
					end
					...
					local custom = json.decode(value) or {}
					if custom then
						TriggerClientEvent("vrp_character:updateCharacter",source,custom,false)
					end
					...
					TriggerEvent("vrp_inventory:Cancel",source)
