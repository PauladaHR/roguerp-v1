...
function startPrisonThreads()
			...
			if distance <= 150 then
				...
				if distance <= 1.5 then
					if IsControlJustPressed(1,38) and prisonTimer <= 0 then
						...
						if not administrative and math.random(1000) > 975 then
							vSERVER.giveKey()
						end
					end
				end
	...
	-- Fugir
	Citizen.CreateThread(function()
		while prison and not administrative do
			local timeDistance = 500
			local ped = PlayerPedId()
			local coords = GetEntityCoords(ped)
...
-- PRESSPRISON
-----------------------------------------------------------------------------------------------------------------------------------------
function hiro.startPrison(status, update, adm)
	if not prison then
		prison = true
		startPrisonThreads()
	end
...
AddEventHandler("police:openSystem",function(doors)
...
RegisterNUICallback("requestGetInfos",function(data,cb)
    local passaporte,nome,sobrenome,bank,phone,identity,fines,weaponport = vSERVER.getInfos(data["passaporte"])
	local genero = vSERVER.getGenero(data["passaporte"])
	if weaponport == 1 then
		weaponporte = "Sim"
	else
		weaponporte = "Não"
	end
    if passaporte then
        cb({ passaporte = passaporte, nome = nome, sobrenome = sobrenome, bank = bank, phone = phone, identity = identity, genero = genero, fines = fines, weaponporte = weaponporte })
    end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- UPDATE
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNetEvent("vrp_mdt:Update")
AddEventHandler("vrp_mdt:Update",function(action)
	SendNUIMessage({ action = action })
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- FINE
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNUICallback("fineUser",function(data,cb)
	if data.deposito then
		vSERVER.fineUser(data.deposito,data.chave,data.texto)
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- PRISON
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNUICallback("prisonUser",function(data,cb)
	if data.passaporte then
		vSERVER.prisonUser(data.passaporte,data.servicos,data.valormulta,data.texto)
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- OCCURRENCE
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNUICallback("occurrenceUser",function(data,cb)
	if data.passaporte then
		vSERVER.occurrenceUser(data.passaporte,data.texto)
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- WEAPON PORT
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNUICallback("weaponPortUser",function(data,cb)
	if data.passaporte then
		vSERVER.weaponPortUser(data.passaporte)
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- TICKET
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNUICallback("delPrison",function(data,cb)
	if data.passaporte then
		vSERVER.delPrisonUser(data.passaporte,data.servicos)
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- REQUESTFINES
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNUICallback("requestArrest",function(data,cb)
	local resultado = vSERVER.getArrests(data.passaporte)
	if resultado then
...
RegisterNUICallback("requestFines",function(data,cb)
	local resultado = vSERVER.getFines(data.passaporte)
	if resultado then
		cb(json.encode(resultado))
	end
end)
...
-- REQUESTOCCURRENCE
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNUICallback("requestOccurrence",function(data,cb)
	local resultado = vSERVER.getOccurrence()
	if resultado then
		cb(json.encode(resultado))
	end
end)
...
-- REQUESTVEHS
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNUICallback("requestVehs",function(data,cb)
	local resultado = vSERVER.getVehs(data.passaporte)
	if resultado then
		cb(json.encode(resultado))
	end
end)
...
-- REQUESTVEHS
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNUICallback("requestHomes",function(data,cb)
	local resultado = vSERVER.getHomes(data.passaporte)
	if resultado then
		cb(json.encode(resultado))
	end
end)
...
RegisterNUICallback("getPermission",function(data,cb)
    local result = vSERVER.checkPermission2()
    cb({ result = result })
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- ANUNCIO ADM
...
-- ANUNCIO ADM
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNUICallback("ContractPolice",function(data,cb)
	if data.passaporte then
		vSERVER.contractPolice(data.passaporte)
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- ANUNCIO ADM
...
RegisterNUICallback("anuncioAdm",function(data,cb)
	if data.texto then
		vSERVER.anuncioPolice(data.texto)
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- REQUESTANNOUNCE
...
RegisterNetEvent("prison:fugitive:setArea", function(x, y, z)
	local blip = AddBlipForRadius(x, y, z, 300.0)
