window.addEventListener("message",function(event){
	...
	if (item["response"] == "openTarget"){
		...
		$(".target-eye").css("color","#C72C41");

	} else if (item["response"] == "leftTarget"){

		$(".target-label").html("");
		$(".target").css("display","none");
		$.post("http://vrp_target/closeTarget");
		$(".target-eye").css("color","#ccc");
...
$(document).on("mousedown",(event) => {
	...
	if (element["id"].split("-")[0] === "target"){
		...
		$.post("http://vrp_target/selectTarget",JSON.stringify({ event: targetData, tunnel: tunnelData, service: serviceData }));
