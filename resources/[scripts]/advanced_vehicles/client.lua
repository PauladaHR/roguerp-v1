...
Citizen.CreateThread(function()
	...
	while true do
		...
		if Config.debugClass then
			print("Vehicle class:", GetVehicleClass(veh))
		end
		if IsEntityAVehicle(veh) and Config.enabledVehicles[GetVehicleClass(veh)] then
			if seat == ped and vehicleData == nil then
				...
				local _,_,_,vname = vRP.vehList()
				if vname then
					vehicleData.name = vname
				else
				...
				if lastPlate ~= vehicleData.plate then
					lastPlate = vehicleData.plate
...
AddEventHandler('advanced_vehicles:removeUpgrade', function(upgrade)
	if upgrade.type == "CHandlingData" then
		...
		if upgrade.sound then
			TriggerServerEvent("advanced_vehicles:syncEngineSound", "DEFAULT", NetworkGetNetworkIdFromEntity(vehicleDataAction.veh))
		end
...
function applyVehicleMods(veh,name,upgrades)
	...
	if name and upgrades then
		...
		for k,v in pairs(upgrades) do
			...
			-- Set engine sound
			if arr[k].improvements.sound then
				TriggerServerEvent("advanced_vehicles:syncEngineSound", arr[k].improvements.sound, NetworkGetNetworkIdFromEntity(veh))
			end
		end
	end
end

RegisterNetEvent("advanced_vehicles:syncEngineSound_c")
AddEventHandler("advanced_vehicles:syncEngineSound_c", function(sound, vehId)
	ForceVehicleEngineAudio(NetworkGetEntityFromNetworkId(vehId), sound)
end)
...
AddEventHandler('advanced_vehicles:useTheJackFunction', function(data,firstStep)
	if firstStep then
		...
		while veh == 0 and distance < 6.0 do
			vehCoords = GetEntityCoords(vehicleDataAction.veh)
...
Citizen.CreateThread(function ()
  local function BoostLoop()
    ...
    if not IsVehicleStopped(vehicle) then
      ...
      local multiplier = Config.NitroForce * maximumSpeed / currentSpeed
