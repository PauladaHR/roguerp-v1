...
$(document).ready(function(){
	...
	document.onkeyup = data => {
		const key = data.key;
		if (key === "Escape"){
			$.post("http://vrp_homes/invClose",JSON.stringify({}));
		} else if (key === "Shift"){
			shiftPressed = false;
		}
	};
	...
	$('.empty').droppable({
		...
		drop: function(event,ui){
			...
			if (origin === "invLeft" && tInv === "invLeft"){
				...
				$.post("http://vrp_homes/updateChest",JSON.stringify({
					item: itemData.key,
					slot: itemData.slot,
					target: target,
					amount: parseInt(amount)
...
const requestChest = () => {
	$.post("http://vrp_homes/requestChest",JSON.stringify({}),(data) => {
		...
		for (let x = 1; x <= 70; x++){
			...
			if (data.myInventory[slot] !== undefined){
				...
				url('http://131.196.196.218/inventory/weapons/${v["index"]}.png'); background-position: center; background-repeat: no-repeat;" data-amount="${v.amount}" data-peso="${v.peso}" data-item-key="${v.key}" data-name-key="${v.name}" data-slot="${slot}" data-description="${v["desc"]}" data-economy="${v["economy"]}">
