	...
	while true do
		...
		if IsPedInAnyVehicle(ped) then
			...
			if IsVehicleEngineOn(vehicle) and GetPedInVehicleSeat(vehicle,-1) == ped and speed >= 5 and GetVehicleFuelLevel(vehicle) >= 2 then
				...
				TriggerServerEvent("vrp_engine:tryFuel",playerAround,VehToNet(vehicle),GetVehicleFuelLevel(vehicle) - (vehFuel[floor(GetVehicleCurrentRpm(vehicle))] or 1.0) * (vehClass[GetVehicleClass(vehicle)] or 1.0) / 10)
			end
		end
		...
		if not IsPedInAnyVehicle(ped) then
			if GetSelectedPedWeapon(ped) == 883325847 then
					...
					if distance <= 3.5 then
						...
						if not isFuel then
							if vehFuel >= 100.0 then
								drawText3D(coordsVeh.x,coordsVeh.y,coordsVeh.z+1,"~p~TANQUE CHEIO",370)
							elseif GetAmmoInPedWeapon(ped,883325847) - 0.02 * 100 <= 0 then
								drawText3D(coordsVeh.x,coordsVeh.y,coordsVeh.z+1,"~b~GALÃO VAZIO",370)
							else
								drawText3D(coordsVeh.x,coordsVeh.y,coordsVeh.z+1,"~b~E~w~   ABASTECER",370)
							end
						else
							if vehFuel >= 0.0 and GetAmmoInPedWeapon(ped,883325847) - 0.02 * 100 > 0 then
								...
								drawText3D(coordsVeh.x,coordsVeh.y,coordsVeh.z+1,"~b~E~w~   CANCELAR",370)
								drawText3D(coordsVeh.x,coordsVeh.y,coordsVeh.z+0.85,"TANQUE: ~y~"..parseInt(floor(vehFuel)).."%   ~w~GALÃO: ~y~"..parseInt(GetAmmoInPedWeapon(ped,883325847) / 4500 * 100).."%",370)
								...
								if vehFuel >= 100.0 or GetAmmoInPedWeapon(ped,883325847) - 0.02 * 100 <= 0 or GetEntityHealth(ped) <= 101 then
									...
									TriggerServerEvent("vrp_engine:tryFuel",playerAround,VehToNet(vehicle),vehFuel)
						...
						if IsControlJustPressed(1,38) and cooldown < 1 then
							cooldown = 5
							if isFuel then
								local playerAround = {}
								for _,player in ipairs(GetActivePlayers()) do
						...
						if DoesEntityExist(vehicle) then
							...
							if distance <= 3.5 then
								...
								if not isFuel then
									...
									if vehFuel >= 0.0 then
										isPrice = isPrice + 0.045
										isGallons = isGallons + 0.02
										SetVehicleFuelLevel(vehicle,vehFuel+0.02)
										drawText3D(coordsVeh.x,coordsVeh.y,coordsVeh.z+1,"~b~E~w~   CANCELAR",370)
										drawText3D(coordsVeh.x,coordsVeh.y,coordsVeh.z+0.85,"TANQUE: ~y~"..parseInt(floor(vehFuel)).."%   ~w~PREÇO: ~y~$"..parseInt(isPrice),370)
										...
										if vehFuel >= 100.0 or GetEntityHealth(ped) <= 101 then
											...
											if vSERVER.paymentFuel(isPrice) then
												...
												TriggerServerEvent("vrp_engine:tryFuel",playerAround,VehToNet(vehicle),lastFuel)
											end
				...
				for k,v in pairs(gallonLocs) do
					...
					if distance <= 3.5 then
						timeDistance = 4
						DrawMarker(21,v[2],v[3],v[4]-0.5,0,0,0,0,180.0,130.0,0.6,0.8,0.5,42,137,255,100,0,0,0,1)
...
CreateThread(function()
	while true do
		...
		if IsPedInAnyVehicle(ped) then
			local vehicle = GetVehiclePedIsUsing(ped)
			local speed = GetEntitySpeed(vehicle) * 2.236936
			if vehFuels[VehToNet(vehicle)] and speed > 0 then
...
-- DRAWTEXT3D
-----------------------------------------------------------------------------------------------------------------------------------------
function drawText3D(x,y,z,text,h)
...
AddEventHandler("engine:vehTuning",function()
	...
	if vehicle and NetworkGetEntityIsNetworked(vehicle) then
		...
		TriggerEvent("Notify","amarelo", "<b>Motor:</b> "..motor.."<br><b>Freio:</b> "..freio.."<br><b>Transmissão:</b> "..transmissao.."<br><b>Suspensão:</b> "..suspensao.."<br><b>Blindagem:</b> "..blindagem.."<br><b>Turbo:</b> "..turbo.."<br><b>Lataria:</b> "..parseInt(body/10).."%<br><b>Motor:</b> "..parseInt(engine/10).."%<br><b>Gasolina:</b> "..parseInt(fuel).."%", 10000, 'info')
	end
end)