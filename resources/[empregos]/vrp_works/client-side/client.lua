...
CreateThread(function()
	while true do
		if GetClockHours() == 7 and GetClockMinutes() == 00 then
			TriggerEvent("Notify","verde", "Jornais Disponiveis!", 5000)
			break
		end
...
-- NEWS
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNetEvent("works:newspaper")
AddEventHandler("works:newspaper",function()
	if GetClockHours() >= 7 and GetClockHours() <= 10 then
        vSERVER.newsPayment()
	else
		TriggerEvent("Notify","amarelo", "Estamos sem jornais no momento.", 5000)
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- TIMESECONDS
