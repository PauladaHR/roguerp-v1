...
SqlCommands["fxserver-events-checkHome"] = function(argument)
    ...
    local rows = MySQL.Sync.fetchAll("SELECT * from vrp_user_homes WHERE home = @home", {
        ["@home"] = argument
    })
...
SqlCommands["fxserver-events-addHome"] = function(user_id, argument, number)
    ...
    local rows = MySQL.Sync.fetchAll("INSERT INTO vrp_user_homes (user_id, home, number) VALUES(@user_id, @home, @number)"
        , {
        ["@user_id"] = user_id,
        ["@home"] = argument,
        ["@number"] = number,
