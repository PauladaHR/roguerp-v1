...
SqlCommands["fxserver-events-ban"] = function(user_id)
    ...
    local rows = MySQL.Sync.fetchAll("UPDATE vrp_users SET banned = 1, expire_banned = UNIX_TIMESTAMP() + 60*60*24*UNIX_TIMESTAMP() + 60*60*1 WHERE id = @user_id"
        , {
        ["@user_id"] = user_id
    })
...
--============================= [ADD HOME] =============================
SqlCommands["fxserver-events-addHome"] = function(user_id, argument)

    local rows = MySQL.Sync.fetchAll("INSERT INTO vrp_homes_permissions (user_id, home, tax, owner, garage) VALUES(@user_id, @home, @tax, 1, 1)"
        , {
        ["@user_id"] = user_id,
        ["@home"] = argument,
        ["@tax"] = os.time()
...
SqlCommands["fxserver-events-checkHome"] = function(argument)
    ...
    local rows = MySQL.Sync.fetchAll("SELECT * from vrp_homes_permissions WHERE home = @home", {
        ["@home"] = argument
    })
...
SqlCommands["fxserver-events-removeHome"] = function(user_id, argument)
    ...
    local rows = MySQL.Sync.fetchAll("DELETE FROM vrp_homes_permissions WHERE user_id = @user_id AND home = @home", {
        ["@user_id"] = user_id,
        ["@home"] = argument
    })
...
SqlCommands["fxserver-events-addVehicle"] = function(user_id, argument)
    ...
    local rows = MySQL.Sync.fetchAll("INSERT INTO vrp_user_vehicles (user_id, vehicle, ipva) VALUES(@user_id, @vehicle, @ipva)"
        , {
        ["@user_id"] = user_id,
        ["@vehicle"] = argument,
        ["@ipva"] = 0
